...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    image_k = eval_kwargs.get('image_k', 50)
    ...
    while True:
        ...
        if sample_n > 1:
            eval_split_n(model, n_predictions, [fc_feats, att_feats, att_masks, data], loader, eval_kwargs)
...
def eval_split_n(model, n_predictions, input_data, loader, eval_kwargs={}):
    verbose = eval_kwargs.get('verbose', True)
    beam_size = eval_kwargs.get('beam_size', 1)
    sample_n = eval_kwargs.get('sample_n', 1)
    ...
    if sample_n_method == 'bs':
        ...
        for k in range(fc_feats.shape[0]):
            max_len = 0
            for temp_i in range(sample_n):
                if max_len < model.done_beams[k][temp_i]['seq'].size(dim=0):
                    max_len = model.done_beams[k][temp_i]['seq'].size(dim=0)
            for temp_i in range(sample_n):
                max_len_temp = max_len - model.done_beams[k][temp_i]['seq'].size(dim=0)
                if max_len_temp:
                    model.done_beams[k][temp_i]['seq'] = torch.cat((model.done_beams[k][temp_i]['seq'], torch.tensor([0 for _ in range(max_len_temp)]).to(device)))
    ...
    # case 2 sample / gumbel / topk sampling/ nucleus sampling
        ...
        for k in range(loader.batch_size):
            max_len = 0
            for temp_i in range(0, sample_n * beam_size, beam_size):
                if max_len < model.done_beams[k][temp_i]['seq'].size(dim=0):
                    max_len = model.done_beams[k][temp_i]['seq'].size(dim=0)
            for temp_i in range(0, sample_n * beam_size, beam_size):
                max_len_temp = max_len - model.done_beams[k][temp_i]['seq'].size(dim=0)
