            ...
            if divm > 0:
                ...
                for prev_choice in range(divm):  # 확률표에 이전 그룹의 단어들에 대한 유사도 가중치 부여
                    ...
                    for prev_labels in range(bdash):  # beam_size 만큼 반복 이전 그룹의 모든 단어를 보기 위함
                        ...
                        # 이전 단어
                        if local_time == 0:
                            top_k_score, top_k_words = logprobs[0].topk(100, 0, True, True)  # 확률표에서 점수가 높은 상위 단어 추출
                            ...
                            for j in top_k_words:  # 추출한 단어들 하나씩 가져오기
                                ...
                                if temp == '0':  # 0이 나올경우 건너뛰기
                                    ...
                                    change[0][j] += hamming_dist(prev_ward, self.vocab[temp]) / top_k_words_len
                        """
                        else:
                            for beam in range(bdash):
                                top_k_score, top_k_words = logprobs[beam].topk(20, 0, True, True)  # 확률표에서 점수가 높은 상위 단어 추출
                                top_k_words_len = len(top_k_words)  # 추출한 단어들의 개수
                                for j in top_k_words:  # 추출한 단어들 하나씩 가져오기
                                    temp = "{}".format(j)  # 추출한 단어 index to string
                                    if temp == '0':  # 0이 나올경우 건너뛰기
                                        ...
                                        change[beam][j] += wordnet_similarity(prev_ward, self.vocab[temp]) / top_k_words_len
                        """

                logprobs = logprobs - change * diversity_lambda / divm
                ...
                if local_time == 0:
                    ...
                    logprobs = logprobs - model_utils.repeat_tensors(bdash, change) * diversity_lambda / divm
                """
